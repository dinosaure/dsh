...
let rec eval
  ...
  | Ast.Ann (loc, e, (lst, ty)) ->
    ...
    (** Infering type annotation `expr : type` is equivalent to inferring the
        type of function call `(� x : type .x)[expr]`, but optimized in
  ...
  | Ast.Case (loc, expr, pattern) ->
    ...
    (fun () ->
       ...
       (* XXX: I think, we close the polymorphic variant too early
          ...
          `f = �x. match x { A() � 0 }
           in g = �x. match x { A() � true | B() � false }
           in �x. (f[x], g[x])` fail in dsh (but not in OCaml).
